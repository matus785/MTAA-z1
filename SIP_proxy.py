...
def init_log( name , host , port):
    if "." in name: 
        name = name.split(".", 1)[0]

    logging.basicConfig(format='%(asctime)s:  %(message)s',filename= name + '.log' ,level=logging.INFO,datefmt='%H:%M:%S')

    logging.info(time.strftime("START | %a, %d %b %Y %H:%M:%S - Server listening", time.localtime()))
    logging.info("IP address: %s , PORT: %s",host,port)

def set_var( host , port ):
    global recordroute 
...
class UDPHandler(socketserver.BaseRequestHandler):   
    ...
    def checkValidity(self,uri):
        ...
        if validity > now:
            ...
            del registrar[uri]
            print("Warninig >> registration for %s has expired" % uri)
            return False
    ...
    def processRegister(self):
        ...
        if expires == 0:
            if registrar.__contains__(fromm):
                del registrar[fromm]
        ...
        logging.info("REGISTER | User: %s - Contact: %s" % (fromm,contact))
        registrar[fromm]=[contact,self.socket,self.client_address,validity]
        self.sendResponse("200 0K")
        
    def processInvite(self):
        origin = self.getOrigin()
        if len(origin) == 0 or not registrar.__contains__(origin):
            self.sendResponse("400 Bad Request")
            return
        destination = self.getDestination()
        if len(destination) > 0:
            if registrar.__contains__(destination) and self.checkValidity(destination):
                ...
                logging.info("CALLING | (from) %s >>> (to) %s" % (origin, destination))
            else:
    ...
    def processAck(self):
        destination = self.getDestination()
        if len(destination) > 0:
            if registrar.__contains__(destination):
                socket,claddr = self.getSocketInfo(destination)
                #self.changeRequestUri()
                self.data = self.addTopVia()
                data = self.removeRouteHeader()
                ...
                text = "\r\n".join(data)
                socket.sendto(text.encode("utf-8"),claddr)
                logging.info("CALL STARTED | (from) %s >>> (to) %s" % (self.getOrigin(), destination))
    ...
    def processBye(self):
        ...
        if len(destination) > 0:
            if registrar.__contains__(destination) and self.checkValidity(destination):
                ...
                text = "\r\n".join(data)
                socket.sendto(text.encode("utf-8") , claddr)
                logging.info("CALL ENDED | (from) %s >>> (to) %s" % (origin, destination))
            else:
                self.sendResponse("406 Not Acceptable")
        else:
            self.sendResponse("500 Server Internal Error")
    ...
    def processCode(self):
        origin = self.getOrigin()
        if len(origin) > 0:
            if registrar.__contains__(origin):
                ...
                if "Decline" in text:
                    text = text.replace("Decline", "Hovor zruseny")
    ...
    def processRequest(self):
        if len(self.data) > 0:
            ...
            print('Processing request: ' + request_uri.rsplit("^")[0])
            ...
            if rx_register.search(request_uri):
                ...
                print ("Request %s unknown" % self.data)
    ...
    def handle(self):
        ...
        if rx_request_uri.search(request_uri) or rx_code.search(request_uri):
            ...
            if len(data) > 4:
                print ("ERROR >> server received [%d]:" % len(data))
                hexdump(data,' ',16)
